...
public class Drone : MonoBehaviour
{
    ...
    // =========================
    // Adaptive Radius (based on speed)
    // =========================
    [Header("Adaptive Radius (Speed-Based)")]
    [Tooltip("Aktifkan radius collider & cast radius yang mengikuti speed (lebih aman saat ngebut).")]
    public bool enableAdaptiveRadius = true;

    [Tooltip("Pilih collider 'outer/buffer' kalau kamu pakai 2 CircleCollider2D. Kalau kosong, akan pakai circleCol default.")]
    public CircleCollider2D outerCircleCollider;

    [Tooltip("Radius minimum saat speed rendah (misal 0.26 sesuai setting kamu).")]
    public float adaptiveRadiusMin = 0.26f;

    [Tooltip("Radius maksimum saat speed tinggi (buffer lebih tebal).")]
    public float adaptiveRadiusMax = 0.34f;

    [Tooltip("Kecepatan yang dianggap 'max' untuk normalisasi. Biasanya = moveSpeed.")]
    public float adaptiveSpeedMax = 2.0f;

    [Tooltip("Seberapa cepat radius berubah (lebih tinggi = lebih cepat respons).")]
    public float adaptiveRadiusLerp = 12f;

    [Tooltip("Kalau true, ambil speed dari rb.velocity.magnitude. Kalau false, pakai moveSpeed.")]
    public bool adaptiveUseRbVelocity = false;

    [Tooltip("Log radius kadang-kadang untuk verifikasi.")]
    public bool logAdaptiveRadius = false;

    private float currentAdaptiveRadius = 0.26f;
    ...
    private void Awake()
    {
        ...
        // fallback: kalau user pakai 2 circle collider, dia bisa drag outerCircleCollider di inspector.
        // kalau tidak, kita pakai circleCol yang terpasang.
        if (outerCircleCollider == null) outerCircleCollider = circleCol;

        // set awal radius adaptif (biar konsisten dari frame 0)
        currentAdaptiveRadius = Mathf.Clamp(adaptiveRadiusMin, 0.01f, 10f);

        // ambil radius collider (untuk CircleCast blocked-ahead)
        if (outerCircleCollider != null)
        {
            float scale = Mathf.Max(transform.lossyScale.x, transform.lossyScale.y);
            colRadius = Mathf.Max(0.01f, outerCircleCollider.radius * scale);
        }
        else
        {
        ...
        // apply adaptive radius immediately
        UpdateAdaptiveRadius(force: true);
    }
    ...
    private void FixedUpdate()
    {
        ...
        // ✅ adaptive radius update (set BEFORE overlap/cast)
        UpdateAdaptiveRadius(force: false);
    ...
    // =========================
    // Adaptive radius impl
    // =========================
    private void UpdateAdaptiveRadius(bool force)
    {
        if (!enableAdaptiveRadius) return;

        float vmax = Mathf.Max(0.01f, adaptiveSpeedMax > 0f ? adaptiveSpeedMax : moveSpeed);

        float speed = adaptiveUseRbVelocity ? rb.linearVelocity.magnitude : moveSpeed;
        float k = Mathf.Clamp01(speed / vmax);

        float targetR = Mathf.Lerp(adaptiveRadiusMin, adaptiveRadiusMax, k);

        float dt = force ? 1f : Time.fixedDeltaTime;
        float lerpT = 1f - Mathf.Exp(-adaptiveRadiusLerp * dt);
        currentAdaptiveRadius = Mathf.Lerp(currentAdaptiveRadius, targetR, lerpT);

        // apply to outer circle collider (buffer)
        if (outerCircleCollider != null)
        {
            // radius di collider adalah local-space, jadi kita bagi skala
            float scale = Mathf.Max(0.0001f, Mathf.Max(transform.lossyScale.x, transform.lossyScale.y));
            outerCircleCollider.radius = Mathf.Max(0.01f, currentAdaptiveRadius / scale);
        }

        // update colRadius used by casts/avoid/overlap
        colRadius = Mathf.Max(0.01f, currentAdaptiveRadius);

        if (logAdaptiveRadius && CanLog())
        {
            Debug.Log($"[Drone:{droneName}] ADAPT_RADIUS speed={speed:F2} k={k:F2} radius={currentAdaptiveRadius:F3} (min={adaptiveRadiusMin:F3} max={adaptiveRadiusMax:F3})");
        }
    }
    ...
    private bool CanLog()
    {
        if (!logState && !logMotion && !logMapping && !logPlanning && !logAdaptiveRadius) return false;
